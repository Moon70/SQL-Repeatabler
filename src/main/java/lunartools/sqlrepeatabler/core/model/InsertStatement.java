...
public class InsertStatement implements Statement{
    ...
    public void toSqlCharacters(SqlBlock sqlBlock){
        ...
        if(!tokenAsString.equalsIgnoreCase("ID") && !tokenAsString.equalsIgnoreCase("[ID]") ){
            tokenFirstColumnName.markWarn();
            logger.warn(String.format("INSERT INTO: Unique key column name is '%s', usually it is 'ID' ! (%s)",tokenFirstColumnName,tokenFirstColumnName.getLocation()));
        }
        ...
        for(int i=0;i<columnValuesTokensList.size();i++) {
            ...
            for(int k=0;k<columnValues.length;k++) {
                System.out.println(">"+columnValues[k]+"<");
                if(columnValues[k].toString().equalsIgnoreCase("now()")) {
                    logger.warn(String.format("It smells like MySql! 'NOW()' is not supported in T-SQL, replaced with 'GETDATE()'. %s",columnValues[k].getLocation()));
                    columnValues[k].markError();
                    columnValues[k]=new Token("GETDATE()",Category.INSERTED);
                    tokenChanged=true;
                }
            }
            if(tokenChanged){
                Token token=new Token("(",Category.INSERTED);
                for(int k=0;k<columnValues.length;k++) {
                    if(k>0) {
                        token.append(", ");
                    }
                    token.append(columnValues[k]);
                }
                token.append(")");
                columnValuesTokensList.set(i, token);
            }
