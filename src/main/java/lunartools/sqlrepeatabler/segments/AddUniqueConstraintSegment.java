...
public class AddUniqueConstraintSegment extends Segment{
	...
	public AddUniqueConstraintSegment(String action,String name,String referencesColumn) {
		super(action,name);
		this.action=action;
		this.name=name;
	...
	public void toSql(StringBuilder sb,TableName tableName,boolean mySql) throws Exception {
		sb.append(String.format("IF OBJECT_ID('%s','UQ') IS NULL", stripDelimiters(name))).append(SqlParser.CRLF);
